...
# Pi3 Training Script with Softplus Activation + mip-NeRF Variance-Weighted Loss
...
# Output directory
OUTPUT_DIR="outputs/pi3_hospital_local_points_mipnerf_variance"
mkdir -p ${OUTPUT_DIR}

echo "=========================================="
echo "Pi3 Training: Softplus + mip-NeRF Variance Loss"
echo "=========================================="
echo "Configuration:"
echo "  - Softplus activation: ENABLED"
...
accelerate launch --config_file configs/accelerate/ddp.yaml \
    ...
    model.depth_activation=softplus \
    ...
    name=pi3_hospital_lowres_mipnerf_variance \
    log.use_wandb=true \
...
LOWRES_CKPT=$(find outputs/pi3_hospital_lowres_mipnerf_variance/ckpts -name "*.pth" | sort -V | tail -n 1)
...
accelerate launch --config_file configs/accelerate/ddp.yaml \
    ...
    +loss.test_loss.lambda_prior=0.5 \
    +loss.test_loss.alpha_clamp=0.3 \
    name=pi3_hospital_highres_mipnerf_variance \
...
HIGHRES_CKPT=$(find outputs/pi3_hospital_highres_mipnerf_variance/ckpts -name "*.pth" | sort -V | tail -n 1)
...
echo "  - Stage 1: outputs/pi3_hospital_lowres_mipnerf_variance/ckpts/"
echo "  - Stage 2: outputs/pi3_hospital_highres_mipnerf_variance/ckpts/"
echo "=========================================="
