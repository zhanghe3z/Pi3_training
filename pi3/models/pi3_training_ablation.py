...
from .layers.transformer_head import LinearPts3d, ContextTransformerDecoder
...
class Pi3Ablation(nn.Module):
    """
    Pi3 Ablation Study Model:
    - Removes Transformer decoder for point prediction
    - Uses only Linear head for point prediction
    """
    def __init__(
            self,
            pos_type='rope100',
        ...
        # ----------------------
        #  ABLATION: Pure Linear Head for Local Points
        #  (No Transformer Decoder)
        # ----------------------
        # Simple projection + LinearPts3d head
        self.point_projection = nn.Linear(2*self.dec_embed_dim, 1024)
        self.point_head = LinearPts3d(patch_size=14, dec_embed_dim=1024, output_dim=3)

        # ----------------------
        #  Camera Pose Decoder (keep original)
        # ----------------------
        # We still use transformer for camera since we're ablating point prediction
        ...
        if train_conf:
            raise NotImplementedError("Confidence training not supported in ablation model")
        ...
        if ckpt is not None:
            checkpoint_dict = torch.load(ckpt, weights_only=False, map_location='cpu')
            # Filter out point_decoder weights since we don't have it
            filtered_checkpoint = {k: v for k, v in checkpoint_dict.items() if not k.startswith('point_decoder')}
            res = self.load_state_dict(filtered_checkpoint, strict=False)
            print(f'[Pi3Ablation] Load checkpoints from {ckpt}: {res}')

            del checkpoint_dict, filtered_checkpoint
    ...
    def forward(self, imgs):
        ...
        # ABLATION: Direct linear projection instead of transformer decoder
        point_hidden = self.point_projection(hidden)
